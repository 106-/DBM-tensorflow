...
class three_layer_1smci:
    ...
    def expectation(self):
        ...
        expectations[1] = tf.reduce_mean(tantan(  upper_signal[:, tf.newaxis, :] -  mid_to_up,  middle_signal[:, :, tf.newaxis] - up_to_mid, self.dbm.weights[1] ), axis=0)
...
class four_layer_1smci:
    ...
    def expectation(self):
        ...
        low_to_mid1  = values[0][:, :, tf.newaxis] * self.dbm.weights[0]
        mid1_to_mid2 = values[1][:, :, tf.newaxis] * self.dbm.weights[1]
        mid2_to_up   = values[2][:, :, tf.newaxis] * self.dbm.weights[2]

        mid1_to_low  = values[1][:, tf.newaxis, :] * self.dbm.weights[0]
        mid2_to_mid1 = values[2][:, tf.newaxis, :] * self.dbm.weights[1]
        up_to_mid2   = values[3][:, tf.newaxis, :] * self.dbm.weights[2]

        expectations[0] = tf.reduce_mean(tantan(   lower_signal[:, :, tf.newaxis] - mid1_to_low,  middle1_signal[:, tf.newaxis, :] -  low_to_mid1, self.dbm.weights[0]), axis=0)
