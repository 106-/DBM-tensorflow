...
class second_smci:
    ...
    def expectation(self):
        ...
        # expectation[0]
        x = (self.dbm.signal(values[0], 1)[:, tf.newaxis, :] - multiply_up[0]) + self.mariginalize2(signals[2][:, tf.newaxis, :] - multiply_up[1], self.dbm.weights[1], axis=2)[:, tf.newaxis, :]
        y = signals[0][:, :, tf.newaxis] - multiply_down[0]
        z = self.dbm.weights[0][tf.newaxis, :, :]
        expectations[0] = self.mariginalize(x, y, z)

        # expectation[1]
        x = (self.dbm.signal(values[2],-2)[:, :, tf.newaxis] - multiply_down[1]) + self.mariginalize2( signals[0][:, :, tf.newaxis] - multiply_down[0], self.dbm.weights[0], axis=1)[:, :, tf.newaxis]
